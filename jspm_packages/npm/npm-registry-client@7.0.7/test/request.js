...
(function(Buffer) {
  ...
  test('request call contract', function(t) {
    t.throws(function() {
      ...
      name: 'AssertionError',
      message: 'must pass uri to request'
    }, 'requires a URI');
    t.throws(function() {
    ...
    t.throws(function() {
      ...
      name: 'AssertionError',
      message: 'must pass params to request'
    ...
    t.throws(function() {
      ...
      name: 'AssertionError',
      message: 'must pass callback to request'
    }, 'requires callback');
    t.throws(function() {
  ...
  test('run request through its paces', function(t) {
    ...
    server.expect('/last-modified', function(req, res) {
      t.equal(req.headers['if-modified-since'], 'test-last-modified', 'got test if-modified-since');
      res.statusCode = 200;
    ...
    client.request(common.registry + '/body-error-string', defaults, function(er) {
      t.equal(er && er.message, 'not really an error unknown: body-error-string', 'call worked');
    });
  ...
  test('outputs notice if npm-notice header is set', function(t) {
    var client = common.freshClient({log: {
        error: noop,
