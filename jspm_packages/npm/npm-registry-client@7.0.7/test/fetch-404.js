...
tap.test('fetch with a 404 response', function(t) {
  ...
  var defaulted = {};
  client.fetch('http://localhost:1337/underscore/-/underscore-1.3.3.tgz', defaulted, function(err, res) {
    t.equal(err.message, 'fetch failed with status code 404', 'got expected error message');
    server.close();
    t.end();
  });
});
