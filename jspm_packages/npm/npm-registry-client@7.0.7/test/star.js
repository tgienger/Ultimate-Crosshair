...
(function(Buffer) {
  ...
  var cache = require("./fixtures/underscore/cache.json!systemjs-json");
  ...
  test('star call contract', function(t) {
    ...
    t.throws(function() {
      ...
      name: 'AssertionError',
      message: 'must pass auth to star'
  ...
  test('star a package', function(t) {
    ...
    server.expect('PUT', '/underscore', function(req, res) {
      ...
      req.on('end', function() {
        ...
        var already = ['vesln', 'mvolkmann', 'lancehunt', 'mikl', 'linus', 'vasc', 'bat', 'dmalam', 'mbrevoort', 'danielr', 'rsimoes', 'thlorenz'];
  ...
  test('if password auth, only sets authorization on put', function(t) {
    var starGet = nock('http://localhost:1010').get('/underscore?write=true').reply(200, {});
    var starPut = nock('http://localhost:1010', {reqheaders: {authorization: 'Basic ' + new Buffer(AUTH.username + ':' + AUTH.password).toString('base64')}}).put('/underscore').reply(200);
    var params = {
      starred: STARRED,
      auth: AUTH
    };
  ...
  test('if token auth, sets bearer on get and put', function(t) {
    var starGet = nock('http://localhost:1010', {reqheaders: {authorization: 'Bearer foo'}}).get('/underscore?write=true').reply(200, {});
    var getUser = nock('http://localhost:1010', {reqheaders: {authorization: 'Bearer foo'}}).get('/-/whoami').reply(200, {username: 'bcoe'});
    var starPut = nock('http://localhost:1010', {reqheaders: {authorization: 'Bearer foo'}}).put('/underscore').reply(200);
    var params = {
      starred: STARRED,
      auth: {token: 'foo'}
    };
