...
test('defaulted initialization', function(t) {
  ...
  var options = client.initialize('http://localhost:1337/', 'GET', 'application/json', {});
  ...
  t.equal(options.headers.version, require("../package.json!systemjs-json").version, 'npm-registry-client version is present in headers');
...
test('initializing with a certificate should map down to the https agent', function(t) {
  ...
  var client = new Client({ssl: {certificate: certificate}});
  var options = client.initialize({protocol: 'https:'}, 'GET', 'application/json', {});
