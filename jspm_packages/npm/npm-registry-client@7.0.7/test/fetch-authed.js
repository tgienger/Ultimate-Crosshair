...
tap.test('basic fetch with scoped always-auth enabled', function(t) {
  server.expect('/underscore/-/underscore-1.3.3.tgz', function(req, res) {
    ...
    t.equal(req.headers.authorization, 'Basic dXNlcm5hbWU6JTEyMzRAYXNkZiU=', 'got expected auth header');
    res.writeHead(200, {
  ...
  client.fetch('http://localhost:1337/underscore/-/underscore-1.3.3.tgz', authed, function(er, res) {
