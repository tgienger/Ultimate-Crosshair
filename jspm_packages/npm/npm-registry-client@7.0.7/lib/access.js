...
subcommands.revoke = function(uri, params, cb) {
  ...
  return this.request(reqUri, {
    method: 'DELETE',
    auth: params.auth,
    body: JSON.stringify({package: params.package})
  }, cb);
};
...
function apiUri(registryUri) {
  var path = Array.prototype.slice.call(arguments, 1).filter(function(x) {
    return x;
  }).map(encodeURIComponent).join('/');
  return url.resolve(registryUri, '-/' + path);
}
function accessAssertions(subcommand, uri, params, cb) {
  assert(subcommands.hasOwnProperty(subcommand), 'access subcommand must be one of ' + Object.keys(subcommands).join(', '));
  typeChecks({
    'uri': [uri, 'string'],
  ...
  if (contains(['public', 'restricted', 'grant', 'revoke', 'ls-collaborators'], subcommand)) {
    typeChecks({'package': [params.package, 'string']});
    assert(!!npa(params.package).scope, 'access commands are only accessible for scoped packages');
  }
  ...
  if (subcommand === 'grant') {
    ...
    assert(params.permissions === 'read-only' || params.permissions === 'read-write', 'permissions must be either read-only or read-write');
  }
}
function typeChecks(specs) {
  Object.keys(specs).forEach(function(key) {
    var checks = specs[key];
    assert(typeof checks[0] === checks[1], key + ' is required and must be of type ' + checks[1]);
  });
}
