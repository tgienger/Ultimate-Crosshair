...
function publish(uri, params, cb) {
  ...
  assert((!access) || ['public', 'restricted'].indexOf(access) !== -1, "if present, access level must be either 'public' or 'restricted'");
  ...
  assert(metadata && typeof metadata === 'object', 'must pass package metadata to publish');
  try {
    fixer.fixNameField(metadata, {
      strict: true,
  ...
  if (!version)
    return cb(new Error('invalid semver: ' + metadata.version));
