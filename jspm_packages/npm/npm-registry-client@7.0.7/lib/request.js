...
(function(Buffer) {
  ...
  function makeRequest(uri, params, cb_) {
    ...
    if (er)
      return cb_(er);
    var opts = this.initialize(parsed, params.method, 'application/json', headers);
  ...
  function requestDone(method, where, cb) {
    return function(er, response, data) {
      ...
      if (!data && response.statusCode >= 400) {
        ...
        return cb(makeError(code + ' ' + STATUS_CODES[code], null, code), null, data, response);
      }
      er = null;
      ...
      if (parsed && parsed.error && typeof parsed.error !== 'object' || response.statusCode >= 400) {
        ...
        if (!parsed.error) {
          er = makeError('Registry returned ' + response.statusCode + ' for ' + method + ' on ' + where, name, response.statusCode);
        } else if (name && parsed.error === 'not_found') {
          er = makeError('404 Not Found: ' + name, name, response.statusCode);
        } else {
          er = makeError(parsed.error + ' ' + (parsed.reason || '') + ': ' + (name || w), name, response.statusCode);
        }
      }
