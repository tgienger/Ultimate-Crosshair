...
(function(process) {
  ...
  function makeRequest(remote, params, cb) {
    ...
    var er = this.authify(params.auth && params.auth.alwaysAuth, parsed, headers, params.auth);
    if (er)
      return cb(er);
    var opts = this.initialize(parsed, 'GET', 'application/x-tar, application/vnd.github+json; q=0.1', headers);
