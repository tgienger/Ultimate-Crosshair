...
(function(process) {
  ...
  test('setup', function(t) {
    mkdirp.sync(pkg);
    fs.writeFileSync(path.join(pkg, 'package.json'), JSON.stringify(json), 'utf8');
    fs.writeFileSync(fixturePath, '//localhost:1337/:_authToken = beeeeeeeeeeeeef\n' + 'registry = http://lvh.me:4321/registry/path\n');
    t.end();
  });
  test('cursory test of unpublishing with config', function(t) {
    ...
    http.createServer(function(req, res) {
      ...
      this.close();
      res.statusCode = 500;
      res.end(JSON.stringify({error: 'shh no tears, only dreams now'}));
      child.kill('SIGHUP');
    }).listen(common.port, function() {
      t.pass('server is listening');
      child = common.npm(['--userconfig', fixturePath, '--loglevel', 'silent', '--force', 'unpublish'], {
        cwd: pkg,
        stdio: 'inherit',
        env: {
