...
test('npm deprecate an unscoped package', function(t) {
  ...
  common.npm(['deprecate', 'cond@0.0.1', 'make it dead', '--registry', common.registry, '--loglevel', 'silent'], {}, function(er, code, stdout, stderr) {
...
test('npm deprecate a scoped package', function(t) {
  ...
  common.npm(['deprecate', '@scope/cond@0.0.1', 'make it dead', '--registry', common.registry, '--loglevel', 'silent'], {}, function(er, code, stdout, stderr) {
    t.ifError(er, 'npm deprecate');
    t.equal(stderr, '', 'no error output');
    t.equal(code, 0, 'exited OK');
    t.end();
...
test('npm deprecate semver range', function(t) {
  ...
  server.get('/cond?write=true').reply(200, cache);
  server.put('/cond', deprecated).reply(201, {deprecated: true});
  common.npm(['deprecate', 'cond@<0.0.2', 'make it dead', '--registry', common.registry, '--loglevel', 'silent'], {}, function(er, code, stdout, stderr) {
    t.ifError(er, 'npm deprecate');
    t.equal(stderr, '', 'no error output');
    t.equal(code, 0, 'exited OK');
    t.end();
...
test('npm deprecate bad semver range', function(t) {
  common.npm(['deprecate', 'cond@-9001', 'make it dead', '--registry', common.registry], {}, function(er, code, stdout, stderr) {
