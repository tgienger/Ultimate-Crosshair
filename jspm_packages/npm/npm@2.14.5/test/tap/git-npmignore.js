...
(function(process) {
  ...
  var expected = ["a.js", "package.json", ".npmignore"].sort();
  ...
  function packInstallTest(spec, t) {
    common.npm(["--loglevel", "silent", "pack", spec], EXEC_OPTS, function(err, code, stdout, stderr) {
      ...
      common.npm(["--loglevel", "silent", "install", packed], EXEC_OPTS, function(err, code, stdout, stderr) {
  ...
  function setup(cb) {
    ...
    common.npm(["--loglevel", "silent", "cache", "clean"], EXEC_OPTS, function(er, code, _, stderr) {
      if (er)
        return cb(er);
      if (code)
        return cb(new Error("npm cache nonzero exit: " + code));
      if (stderr)
        return cb(new Error("npm cache clean error: " + stderr));
      which("git", function found(er, git) {
        ...
        function init(er, _, stderr) {
          if (er)
            return cb(er);
          if (stderr)
            return cb(new Error("git init error: " + stderr));
        ...
        function user(er, _, stderr) {
          if (er)
            return cb(er);
          if (stderr)
            return cb(new Error("git config error: " + stderr));
        ...
        function addAll(er, _, stderr) {
          if (er)
            return cb(er);
          if (stderr)
            return cb(new Error("git add . error: " + stderr));
        ...
        function commit(er, _, stderr) {
          if (er)
            return cb(er);
          if (stderr)
            return cb(new Error("git commit error: " + stderr));
          cb();
        }
      });
    });
