...
(function(process) {
  ...
  var desired = {
    ...
    "version": "0.0.0",
    "dependencies": {"npm-test-shrinkwrap-local-dependency-dep": {
        "version": "0.0.0",
        "from": "dep",
        "resolved": "file:dep"
      }}
  ...
  var root = {
    ...
    "name": "npm-test-shrinkwrap-local-dependency",
    "version": "0.0.0",
    "dependencies": {"npm-test-shrinkwrap-local-dependency-dep": "file:./dep"}
  };
  var dependency = {
    "author": "Thomas Torp",
  ...
  test("shrinkwrap uses resolved with file: on local deps", function(t) {
    setup();
    common.npm(["--cache=" + CACHE_DIR, "--loglevel=silent", "install", "."], {}, function(err, code) {
      t.ifError(err, "npm install worked");
      t.equal(code, 0, "npm exited normally");
      common.npm(["--cache=" + CACHE_DIR, "--loglevel=silent", "shrinkwrap"], {}, function(err, code) {
  ...
  test('"npm install" should install local packages from shrinkwrap', function(t) {
    ...
    common.npm(["--cache=" + CACHE_DIR, "--loglevel=silent", "install", "."], {}, function(err, code) {
      ...
      var dependencyPackageJson = path.resolve(PKG_DIR, "node_modules/npm-test-shrinkwrap-local-dependency-dep/package.json");
      t.ok(JSON.parse(fs.readFileSync(dependencyPackageJson, "utf8")), "package with local dependency installed from shrinkwrap");
      t.end();
    });
  });
  ...
  function setup() {
    ...
    mkdirp.sync(DEP_DIR);
    fs.writeFileSync(path.resolve(PKG_DIR, "package.json"), JSON.stringify(root, null, 2));
    fs.writeFileSync(path.resolve(DEP_DIR, "package.json"), JSON.stringify(dependency, null, 2));
