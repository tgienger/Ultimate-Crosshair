...
(function(process) {
  ...
  test('\'npm install --save --save-exact\' should install local pkg', function(t) {
    common.npm(['--loglevel', 'silent', '--registry', common.registry, '--save', '--save-exact', 'install', 'underscore@1.3.1'], EXEC_OPTS, function(err, code) {
      ...
      t.same(pkgJson.dependencies, {'underscore': '1.3.1'}, 'underscore dependency should specify exactly 1.3.1');
      t.end();
    });
  });
  test('\'npm install --save-dev --save-exact\' should install local pkg', function(t) {
    setup();
    common.npm(['--loglevel', 'silent', '--registry', common.registry, '--save-dev', '--save-exact', 'install', 'underscore@1.3.1'], EXEC_OPTS, function(err, code) {
      t.ifError(err, 'npm ran without issue');
      t.notOk(code, 'npm install exited without raising an error code');
      var p = path.resolve(pkg, 'node_modules/underscore/package.json');
      t.ok(JSON.parse(fs.readFileSync(p)));
      p = path.resolve(pkg, 'package.json');
      var pkgJson = JSON.parse(fs.readFileSync(p, 'utf8'));
      t.same(pkgJson.devDependencies, {'underscore': '1.3.1'}, 'underscore dependency should specify exactly 1.3.1');
      t.end();
    });
  });
