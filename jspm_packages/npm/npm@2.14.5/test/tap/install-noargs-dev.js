...
(function(process) {
  ...
  var PACKAGE_JSON2 = {
    name: 'install-noargs-dev',
    version: '0.0.2',
    devDependencies: {'underscore': '1.5.1'}
  };
  ...
  test('install noargs installs updated devDependencies', function(t) {
    fs.writeFileSync(path.join(pkg, 'package.json'), JSON.stringify(PACKAGE_JSON2, null, 2));
    common.npm(['--registry', common.registry, '--loglevel', 'silent', 'install'], EXEC_OPTS, function(err, code) {
      t.ifError(err, 'npm install ran without issue');
      t.notOk(code, 'npm install exited with code 0');
      var p = path.join(pkg, 'node_modules', 'underscore', 'package.json');
  ...
  function setup() {
    ...
    fs.writeFileSync(path.join(pkg, 'package.json'), JSON.stringify(PACKAGE_JSON1, null, 2));
    process.chdir(pkg);
  }
})(require("process"));
