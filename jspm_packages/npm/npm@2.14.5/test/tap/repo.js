...
(function(process) {
  ...
  test("npm repo underscore", function(t) {
    mr({port: common.port}, function(er, s) {
      common.npm(['repo', 'underscore', '--registry=' + common.registry, '--loglevel=silent', '--browser=' + __dirname + '/_script.sh'], opts, function(err, code, stdout, stderr) {
  ...
  test('npm repo optimist - github (https://)', function(t) {
    mr({port: common.port}, function(er, s) {
      common.npm(['repo', 'optimist', '--registry=' + common.registry, '--loglevel=silent', '--browser=' + __dirname + '/_script.sh'], opts, function(err, code, stdout, stderr) {
        t.equal(code, 0, 'exit ok');
        var res = fs.readFileSync(outFile, 'ascii');
        s.close();
  ...
  test("npm repo npm-test-peer-deps - no repo", function(t) {
    mr({port: common.port}, function(er, s) {
      common.npm(['repo', 'npm-test-peer-deps', '--registry=' + common.registry, '--loglevel=silent', '--browser=' + __dirname + '/_script.sh'], opts, function(err, code, stdout, stderr) {
  ...
  test("npm repo test-repo-url-http - non-github (http://)", function(t) {
    mr({port: common.port}, function(er, s) {
      common.npm(['repo', 'test-repo-url-http', '--registry=' + common.registry, '--loglevel=silent', '--browser=' + __dirname + '/_script.sh'], opts, function(err, code, stdout, stderr) {
        t.equal(code, 0, 'exit ok');
        var res = fs.readFileSync(outFile, 'ascii');
        s.close();
  ...
  test("npm repo test-repo-url-https - non-github (https://)", function(t) {
    mr({port: common.port}, function(er, s) {
      common.npm(['repo', 'test-repo-url-https', '--registry=' + common.registry, '--loglevel=silent', '--browser=' + __dirname + '/_script.sh'], opts, function(err, code, stdout, stderr) {
        t.equal(code, 0, 'exit ok');
        var res = fs.readFileSync(outFile, 'ascii');
        s.close();
  ...
  test("npm repo test-repo-url-ssh - non-github (ssh://)", function(t) {
    mr({port: common.port}, function(er, s) {
      common.npm(['repo', 'test-repo-url-ssh', '--registry=' + common.registry, '--loglevel=silent', '--browser=' + __dirname + '/_script.sh'], opts, function(err, code, stdout, stderr) {
        t.equal(code, 0, 'exit ok');
        var res = fs.readFileSync(outFile, 'ascii');
        s.close();
