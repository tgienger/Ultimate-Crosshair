...
(function(process) {
  ...
  var dependency = {
    name: 'dep',
    version: '1.0.0',
    scripts: {install: './bin/foo'}
  };
  var foo = function() {}.toString().split('\n').slice(1, -1).join('\n');
  process.env.npm_config_prefix = tmp;
  test('plain install', function(t) {
    setup();
    common.npm(['install', dep, '--tmp', tmp], {cwd: pkg}, function(err, code, stdout, stderr) {
  ...
  test('link', function(t) {
    setup();
    common.npm(['link', '--tmp', tmp], {cwd: dep}, function(err, code, stdout, stderr) {
  ...
  test('install --link', function(t) {
    ...
    common.npm(['link', '--tmp', tmp], {cwd: dep}, function(err, code, stdout, stderr) {
      t.ifErr(err, 'npm link finished without error');
      common.npm(['install', '--link', dependency.name, '--tmp', tmp], {cwd: pkg}, function(err, code, stdout, stderr) {
  ...
  function setup() {
    ...
    fs.writeFileSync(path.join(dep, 'package.json'), JSON.stringify(dependency, null, 2));
