...
(function(process) {
    ...
    npm.load({
      ...
      common.makeGitRepo({path: pkg}, function(err, git) {
        ...
        version(['patch'], function(err) {
          ...
          t.deepEqual(readStatus('preversion', t), {'preversion-package.json': 'A'});
          ...
          t.deepEqual(readStatus('postversion', t), {'postversion-package.json': 'A'});
          t.end();
        });
      });
    });
  ...
  function makeScript(lifecycle) {
    var contents = ['cp package.json ' + lifecycle + '-package.json', 'git add ' + lifecycle + '-package.json', 'git status --porcelain > ' + lifecycle + '-git.txt'].join('\n');
  ...
  function readStatus(lifecycle, t) {
    var status = {};
    fs.readFileSync(path.join(pkg, lifecycle + '-git.txt'), 'utf-8').trim().split('\n').forEach(function(line) {
      line = line.trim();
      if (line && !line.match(/^\?\? /)) {
