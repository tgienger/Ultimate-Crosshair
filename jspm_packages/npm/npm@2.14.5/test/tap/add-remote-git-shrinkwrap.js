...
(function(process) {
  ...
  var pjParent = JSON.stringify({
    name: 'parent',
    version: '1.2.3',
    dependencies: {'child': 'git://localhost:1235/child.git#master'}
  }, null, 2) + '\n';
  ...
  test('shrinkwrap gets correct _from and _resolved (#7121)', function(t) {
    common.npm(['shrinkwrap', '--loglevel', 'silent'], {cwd: pkg}, function(er, code, stdout, stderr) {
      ...
      t.equal(shrinkwrap.dependencies.child.from, 'git://localhost:1235/child.git#master', 'npm shrinkwrapped from correctly');
      git.whichAndExec(['rev-list', '-n1', 'master'], {
        cwd: repo,
        env: process.env
        ...
        t.equal(shrinkwrap.dependencies.child.resolved, 'git://localhost:1235/child.git#' + treeish, 'npm shrinkwrapped resolved correctly');
        t.end();
      });
    });
  });
  ...
  function setup(cb) {
    ...
    npm.load({
      ...
      function startDaemon(cb) {
        var d = git.spawn(['daemon', '--verbose', '--listen=localhost', '--export-all', '--base-path=.', '--port=1235'], {
          cwd: pkg,
          env: process.env,
      ...
      common.makeGitRepo({
        path: repo,
        commands: [git.chainableExec(['clone', '--bare', repo, 'child.git'], {
          cwd: pkg,
          env: process.env
        }), startDaemon]
      }, cb);
