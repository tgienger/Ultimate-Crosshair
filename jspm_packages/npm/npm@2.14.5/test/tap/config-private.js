...
test("config get private var (old auth)", function(t) {
  common.npm(["config", "get", "_auth"], opts, function(err, code, stdout, stderr) {
    t.ifError(err);
...
test("config get private var (new auth)", function(t) {
  common.npm(["config", "get", "//registry.npmjs.org/:_password"], opts, function(err, code, stdout, stderr) {
    t.ifError(err);
    t.similar(stderr, /sekretz/, "password blocked on stderr");
    t.equal(stdout, "", "no output");
    t.end();
...
test("config get public var (new username)", function(t) {
  ...
  common.npm(["config", "get", "//registry.lvh.me/:username", "--userconfig=" + FIXTURE_PATH, "--registry=http://registry.lvh.me/"], opts, function(err, code, stdout, stderr) {
    t.ifError(err);
