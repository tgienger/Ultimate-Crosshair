...
var manifest = require("../../package.json!systemjs-json");
...
test("all deps are bundled deps or dev deps", function(t) {
  deps.forEach(function(name) {
    t.assert(bundled.indexOf(name) !== -1, name + " is in bundledDependencies");
  });
  t.same(fs.readdirSync(path.resolve(__dirname, "../../node_modules")).filter(function(name) {
