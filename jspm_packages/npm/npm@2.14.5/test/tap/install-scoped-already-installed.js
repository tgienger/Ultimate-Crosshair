...
(function(process) {
  ...
  test('setup', function(t) {
    rimraf.sync(root);
    mkdirp.sync(pkg);
    fs.writeFileSync(path.join(pkg, 'package.json'), JSON.stringify(scopedPaths, null, 2));
    ...
    fs.writeFileSync(path.join(root, 'package-scoped-dependency', 'package.json'), JSON.stringify(scopedDependency, null, 2));
    process.chdir(pkg);
    t.end();
  });
  test('installing already installed local scoped package', function(t) {
    common.npm(['--loglevel', 'silent', 'install'], EXEC_OPTS, function(err, code, stdout) {
      var installed = parseNpmInstallOutput(stdout);
      t.ifError(err, 'install ran to completion without error');
      t.notOk(code, 'npm install exited with code 0');
      t.ok(existsSync(path.join(modules, '@scoped', 'package', 'package.json')), 'package installed');
