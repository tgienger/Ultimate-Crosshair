...
(function(process) {
  ...
  var pjParent = JSON.stringify({
    name: 'parent',
    version: '1.2.3',
    dependencies: {child: 'git://localhost:1233/child.git'}
  }, null, 2) + '\n';
  ...
  function setup(cb) {
    ...
    npm.load({
      ...
      require("module")._cache[require.resolve('which')] = {exports: function(_, cb) {
          cb();
        }};
      git = require("../../lib/utils/git");
      function startDaemon(cb) {
        var d = git.spawn(['daemon', '--verbose', '--listen=localhost', '--export-all', '--base-path=.', '--port=1233'], {
          cwd: pkg,
          env: process.env,
