...
test('not every pkg.name can be required', function(t) {
  common.npm(['install', '.', '--cache', cache, '--registry', common.registry], EXEC_OPTS, function(err, code) {
    t.ifErr(err, 'install finished without error');
    t.equal(code, 0, 'install exited ok');
    t.ok(existsSync(path.join(pkg, 'node_modules/test-package-with-one-dep', 'node_modules/test-package')), 'package installed OK');
    t.end();
  });
});
