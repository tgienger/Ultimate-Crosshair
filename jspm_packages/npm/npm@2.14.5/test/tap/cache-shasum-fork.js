...
(function(process) {
  ...
  var forkPath = path.resolve(__dirname, '..', 'fixtures', 'forked-underscore-1.5.1.tgz');
  ...
  test('npm cache - install from fork', function(t) {
    setup();
    common.npm(['--loglevel', 'silent', '--registry', common.registry, 'install', forkPath], {
      cwd: pkg,
      ...
      var index = fs.readFileSync(path.join(pkg, 'node_modules', 'underscore', 'index.js'), 'utf8');
  ...
  test('npm cache - install from origin', function(t) {
    setup();
    common.npm(['--loglevel', 'silent', '--registry', common.registry, 'install', 'underscore'], {
      cwd: pkg,
      env: {npm_config_cache: cache}
    }, function(err, code, stdout, stderr) {
      t.ifErr(err, 'install finished without error');
