...
(function(process) {
  ...
  test('installing a package that depends on the current package', function(t) {
    common.npm(['--registry', common.registry, '--loglevel', 'silent', 'install', 'optimist'], EXEC_OPTS, function(err, code, stdout, stderr) {
      t.ifError(err, 'npm ran without issue');
      t.notOk(code, 'npm ran without raising an error code');
      t.notOk(stderr, 'no error output');
      common.npm(['--registry', common.registry, '--loglevel', 'silent', 'dedupe'], EXEC_OPTS, function(err, code, stdout, stderr) {
        t.ifError(err, 'npm ran without issue');
        t.notOk(code, 'npm ran without raising an error code');
        t.notOk(stderr, 'no error output');
        t.ok(existsSync(path.resolve(minimist, 'node_modules', 'optimist', 'node_modules', 'minimist')), 'circular dependency uncircled');
        t.end();
      });
    });
  });
  ...
  function setup(cb) {
    ...
    fs.writeFileSync(path.join(minimist, 'package.json'), JSON.stringify(json, null, 2));
