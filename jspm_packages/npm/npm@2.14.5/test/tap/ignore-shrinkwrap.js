...
(function(process) {
  ...
  var json = {
    ...
    version: '0.0.0',
    dependencies: {'npm-test-ignore-shrinkwrap-file': 'http://localhost:1337/package.js'}
  };
  var shrinkwrap = {
    name: 'ignore-shrinkwrap',
    version: '0.0.0',
    dependencies: {'npm-test-ignore-shrinkwrap-file': {
        version: '1.2.3',
  ...
  test('npm install --no-shrinkwrap', function(t) {
    mr({
      ...
      common.npm(['--registry', common.registry, '--loglevel', 'silent', 'install', '--no-shrinkwrap'], EXEC_OPTS, function(err, code) {
  ...
  test('npm install (with shrinkwrap)', function(t) {
    mr({
      ...
      common.npm(['--registry', common.registry, '--loglevel', 'silent', 'install'], EXEC_OPTS, function(err, code) {
        t.ifError(err, 'npm ran without issue');
        t.ok(code, "install isn't going to succeed");
        s.close();
        t.end();
  ...
  function setup() {
    ...
    fs.writeFileSync(path.join(pkg, 'npm-shrinkwrap.json'), JSON.stringify(shrinkwrap, null, 2));
    process.chdir(pkg);
  }
})(require("process"));
