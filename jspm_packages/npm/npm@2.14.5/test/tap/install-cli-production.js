...
(function(process) {
  ...
  var json = {
    ...
    description: 'fixture',
    version: '0.0.0',
    scripts: {prepublish: 'exit 123'},
    dependencies: {dependency: 'file:./dependency'},
    devDependencies: {'dev-dependency': 'file:./dev-dependency'}
  };
  var dependency = {
    name: 'dependency',
    description: 'fixture',
  ...
  test('setup', function(t) {
    ...
    fs.writeFileSync(path.join(pkg, 'dependency', 'package.json'), JSON.stringify(dependency, null, 2));
    mkdirp.sync(path.join(pkg, 'devDependency'));
    fs.writeFileSync(path.join(pkg, 'devDependency', 'package.json'), JSON.stringify(devDependency, null, 2));
  ...
  test('\'npm install --production\' should only install dependencies', function(t) {
    common.npm(['install', '--production'], EXEC_OPTS, function(err, code) {
      ...
      t.ok(JSON.parse(fs.readFileSync(path.resolve(pkg, 'node_modules/dependency/package.json'), 'utf8')), 'dependency was installed');
      t.notOk(existsSync(path.resolve(pkg, 'node_modules/dev-dependency/package.json')), 'devDependency was NOT installed');
      t.end();
    });
  });
