...
(function(process) {
  ...
  test("install npm into first folder", function(t) {
    var args = ["install", npm, "-g", "--prefix=" + folder + "/first", "--ignore-scripts", "--cache=" + folder + "/cache", "--no-spin", "--loglevel=silent", "--tmp=" + folder + "/tmp"];
  ...
  test("write npmrc file", function(t) {
    common.npm(["explore", "npm", "-g", "--prefix=" + folder + "/first", "--cache=" + folder + "/cache", "--tmp=" + folder + "/tmp", "--no-spin", "--", node, __filename, "write-builtin", process.pid], {"stdio": "inherit"}, function(er, code) {
      if (er)
        throw er;
      t.equal(code, 0);
      t.end();
  ...
  test("use first npm to install second npm", function(t) {
    common.npm(["root", "-g", "--prefix=" + folder + "/first", "--cache=" + folder + "/cache", "--tmp=" + folder + "/tmp", "--no-spin"], {}, function(er, code, so) {
      if (er)
        throw er;
      t.equal(code, 0);
      var root = so.trim();
      ...
      spawn(node, [bin, "install", npm, "-g", "--prefix=" + folder + "/second", "--cache=" + folder + "/cache", "--tmp=" + folder + "/tmp", "--no-spin"]).on("error", function(er) {
        throw er;
  ...
  test("verify that the builtin config matches", function(t) {
    common.npm(["root", "-g", "--prefix=" + folder + "/first", "--cache=" + folder + "/cache", "--tmp=" + folder + "/tmp"], {}, function(er, code, so) {
      if (er)
        throw er;
      t.equal(code, 0);
      var firstRoot = so.trim();
      common.npm(["root", "-g", "--prefix=" + folder + "/second", "--cache=" + folder + "/cache", "--tmp=" + folder + "/tmp"], {}, function(er, code, so) {
        if (er)
          throw er;
        t.equal(code, 0);
