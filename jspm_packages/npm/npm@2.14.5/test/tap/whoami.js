...
test("npm whoami with basic auth", function(t) {
  ...
  common.npm(["whoami", "--userconfig=" + FIXTURE_PATH, "--registry=http://registry.lvh.me/"], opts, function(err, code, stdout, stderr) {
    t.ifError(err);
...
test("npm whoami with bearer auth", {timeout: 2 * 1000}, function(t) {
  var s = "//localhost:" + common.port + "/:_authToken = wombat-developers-union\n";
  fs.writeFileSync(FIXTURE_PATH, s, "ascii");
  fs.chmodSync(FIXTURE_PATH, "0444");
  ...
  server.listen(common.port, function() {
    common.npm(["whoami", "--userconfig=" + FIXTURE_PATH, "--registry=http://localhost:" + common.port + "/"], opts, function(err, code, stdout, stderr) {
      t.ifError(err);
      t.equal(stderr, "", "got nothing on stderr");
      t.equal(code, 0, "exit ok");
      t.equal(stdout, "wombat\n", "got username");
