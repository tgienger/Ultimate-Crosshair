...
(function(process) {
  ...
  var json = {
    ...
    version: '1.2.3',
    dependencies: {mkdirp: '~0.3.0'},
    devDependencies: {underscore: '~1.3.1'}
  };
  function clone(a) {
    return extend({}, a);
  }
  ...
  test('update regular dependencies only', function(t) {
    ...
    common.npm(['update', '--save'], EXEC_OPTS, function(err, code) {
      ...
      t.deepEqual(pkgdata.devDependencies, json.devDependencies, 'dev dependencies should be untouched');
      t.deepEqual(pkgdata.optionalDependencies, json.optionalDependencies, 'optional dependencies should be untouched');
      t.end();
    });
  });
  test('update devDependencies only', function(t) {
    ...
    common.npm(['update', '--save-dev'], EXEC_OPTS, function(err, code) {
      ...
      t.notOk(code, 'npm update exited with code 0');
      var pkgdata = JSON.parse(fs.readFileSync(path.join(pkg, 'package.json'), 'utf8'));
      t.deepEqual(pkgdata.dependencies, json.dependencies, 'dependencies should be untouched');
  ...
  test('optionalDependencies are merged into dependencies during --save', function(t) {
    ...
    common.npm(['update', '--save'], EXEC_OPTS, function(err, code) {
      ...
      t.deepEqual(pkgdata.devDependencies, cloned.devDependencies, 'dev dependencies should be untouched');
      t.deepEqual(pkgdata.optionalDependencies, cloned.optionalDependencies, 'optional dependencies should be untouched');
      t.end();
    });
  });
  test('semver prefix is replaced with configured save-prefix', function(t) {
    ...
    common.npm(['update', '--save', '--save-prefix', '~'], EXEC_OPTS, function(err, code) {
      ...
      t.notOk(code, 'npm update exited with code 0');
      var pkgdata = JSON.parse(fs.readFileSync(path.join(pkg, 'package.json'), 'utf8'));
      t.deepEqual(pkgdata.dependencies, {mkdirp: '~0.3.5'}, 'dependencies should be updated');
      t.deepEqual(pkgdata.devDependencies, json.devDependencies, 'dev dependencies should be untouched');
      t.deepEqual(pkgdata.optionalDependencies, json.optionalDependencies, 'optional dependencies should be updated');
      t.end();
    });
  });
  ...
  function setup(extendWith) {
    ...
    fs.writeFileSync(path.join(pkg, 'package.json'), JSON.stringify(template, null, 2));
    return template;
  }
})(require("process"));
