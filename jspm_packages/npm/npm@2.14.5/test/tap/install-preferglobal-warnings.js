...
(function(process) {
  ...
  var dependenciesJson = {
    ...
    version: '0.0.0',
    dependencies: {'npm-test-preferglobal-dep': 'file:../' + preferGlobalJson.name}
  };
  var devDependenciesJson = {
    name: 'npm-test-preferglobal-devDependency-check',
    version: '0.0.0',
    devDependencies: {'npm-test-preferglobal-dep': 'file:../' + preferGlobalJson.name}
  };
  var emptyPackage = {
  ...
  test('install a preferGlobal dependency without warning', function(t) {
    ...
    common.npm(['install', '--loglevel=warn'], {}, function(err, code, stdout, stderr) {
      t.ifError(err, 'packages were installed');
      t.notMatch(stderr, /WARN.*prefer global/, 'install should not warn when dependency is preferGlobal');
      t.end();
    });
  });
  test('install a preferGlobal dependency without warning', function(t) {
    ...
    common.npm(['install', '--loglevel=warn'], {}, function(err, code, stdout, stderr) {
      t.ifError(err, 'packages were installed');
      t.notMatch(stderr, /WARN.*prefer global/, 'install should not warn when devDependency is preferGlobal');
      t.end();
    });
  });
  test('warn if a preferGlobal package is being installed direct', function(t) {
    setup(emptyPackage);
    common.npm(['install', 'file:../' + preferGlobalJson.name, '--loglevel=warn'], {}, function(err, code, stdout, stderr) {
      t.ifError(err, 'packages were installed');
      t.match(stderr, /WARN.*prefer global/, 'install should warn when new package is preferGlobal');
      t.end();
    });
  });
  test('warn if a preferGlobal package is being saved', function(t) {
    setup(emptyPackage);
    common.npm(['install', 'file:../' + preferGlobalJson.name, '--save', '--loglevel=warn'], {}, function(err, code, stdout, stderr) {
      t.ifError(err, 'packages were installed');
      t.match(stderr, /WARN.*prefer global/, 'install should warn when new package is preferGlobal');
      t.end();
    });
  ...
  function cleanup() {
    ...
    var pkgs = [preferGlobalJson, dependenciesJson, devDependenciesJson, emptyPackage];
