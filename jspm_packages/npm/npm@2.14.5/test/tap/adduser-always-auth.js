...
test("npm login", function(t) {
  mr({
    ...
    var runner = common.npm(["login", "--registry", common.registry, "--loglevel", "silent", "--userconfig", outfile], opts, function(err, code) {
...
test("npm login --always-auth", function(t) {
  mr({
    ...
    var runner = common.npm(["login", "--registry", common.registry, "--loglevel", "silent", "--userconfig", outfile, "--always-auth"], opts, function(err, code) {
      t.notOk(code, "exited OK");
      t.notOk(err, "no error output");
      var config = fs.readFileSync(outfile, "utf8");
...
test("npm login --no-always-auth", function(t) {
  mr({
    ...
    var runner = common.npm(["login", "--registry", common.registry, "--loglevel", "silent", "--userconfig", outfile, "--no-always-auth"], opts, function(err, code) {
      t.notOk(code, "exited OK");
      t.notOk(err, "no error output");
      var config = fs.readFileSync(outfile, "utf8");
