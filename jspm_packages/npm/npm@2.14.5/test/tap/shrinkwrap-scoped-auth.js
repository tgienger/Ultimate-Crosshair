...
(function(process) {
  ...
  test("authed npm install with shrinkwrapped scoped package", function(t) {
    common.npm(["install", "--loglevel", "silent", "--fetch-retries", 0, "--userconfig", outfile], EXEC_OPTS, function(err, code, stdout, stderr) {
      t.ifError(err, "test runner executed without error");
      t.equal(code, 0, "npm install exited OK");
      t.notOk(stderr, "no output on stderr");
      t.equal(stdout, "@scoped/underscore@1.3.1 node_modules/@scoped/underscore\n", "module installed where expected");
      t.end();
    });
  });
  ...
  var shrinkwrap = {
    name: "test-package-install",
    version: "1.0.0",
    dependencies: {"@scoped/underscore": {
        resolved: tarballURL,
        from: ">=1.3.1 <2",
        version: "1.3.1"
      }}
  ...
  function setup() {
    ...
    writeFileSync(resolve(pkg, "npm-shrinkwrap.json"), JSON.stringify(shrinkwrap, null, 2) + "\n");
  }
  function cleanup() {
