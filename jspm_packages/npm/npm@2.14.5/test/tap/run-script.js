...
(function(process) {
  ...
  var lifecycleOnly = {
    name: 'scripted',
    version: '1.2.3',
    scripts: {'prestart': 'echo prestart'}
  };
  var directOnly = {
    name: 'scripted',
    version: '1.2.3',
    scripts: {'whoa': 'echo whoa'}
  };
  var both = {
    name: 'scripted',
    version: '1.2.3',
  ...
  var preversionOnly = {
    name: 'scripted',
    version: '1.2.3',
    scripts: {'preversion': 'echo preversion'}
  };
  ...
  function writeMetadata(object) {
    fs.writeFileSync(path.resolve(pkg, 'package.json'), JSON.stringify(object, null, 2) + '\n');
  }
  function cleanup() {
    rimraf.sync(pkg);
  }
  ...
  test('npm run-script no-params (lifecycle only)', function(t) {
    var expected = ['Lifecycle scripts included in scripted:', '  prestart', '    echo prestart', ''].join('\n');
  ...
  test('npm run-script no-params (preversion only)', function(t) {
    var expected = ['Lifecycle scripts included in scripted:', '  preversion', '    echo preversion', ''].join('\n');
  ...
  test('npm run-script no-params (direct only)', function(t) {
    var expected = ['Scripts available in scripted via `npm run-script`:', '  whoa', '    echo whoa', ''].join('\n');
  ...
  test('npm run-script no-params (direct only)', function(t) {
    var expected = ['Lifecycle scripts included in scripted:', '  prestart', '    echo prestart', '', 'available via `npm run-script`:', '  whoa', '    echo whoa', ''].join('\n');
    writeMetadata(both);
    common.npm(['run-script'], opts, function(err, code, stdout, stderr) {
      t.ifError(err, 'ran run-script without parameters without crashing');
      t.notOk(code, 'npm exited without error code');
