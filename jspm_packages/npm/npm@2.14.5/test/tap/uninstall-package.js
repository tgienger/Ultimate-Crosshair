...
(function(process) {
  ...
  test('returns a list of removed items', function(t) {
    mr({port: common.port}, function(er, s) {
      common.npm(['--registry', common.registry, '--loglevel', 'silent', 'install', '.'], EXEC_OPTS, function(err, code, stdout, stderr) {
        t.ifError(err, 'install ran without issue');
        t.notOk(code, 'install ran without raising error code');
        common.npm(['--registry', common.registry, '--loglevel', 'silent', 'uninstall', 'underscore', 'request', 'lala'], EXEC_OPTS, function(err, code, stdout, stderr) {
