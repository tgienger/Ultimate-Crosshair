...
(function(process) {
  ...
  var npm = requireInject.installGlobally('../../lib/npm.js', {'child_process': {'execFile': function(cmd, args, options, cb) {
        if (args[0] === 'clone') {
  ...
  function extract(tarball, target, cb) {
    cb = once(cb);
    fs.createReadStream(tarball).on('error', function(er) {
      cb(er);
    }).pipe(zlib.createGunzip()).on('error', function(er) {
      cb(er);
    }).pipe(tar.Extract({path: target})).on('error', function(er) {
      cb(er);
    }).on('end', function() {
      cb();
    });
  ...
  function gitWhichAndExec(args, options, cb) {
    if (gitcmd)
      return execGit(args, options, cb);
  ...
  function setup(cb) {
    ...
    function andLoadNpm() {
      var opts = {cache: path.resolve(wd, 'cache')};
      npm.load(opts, cb);
    }
  }
  var oneTree = ['npm-git-test@1.0.0', [['dummy-npm-bar@4.0.0', [['dummy-npm-foo@3.0.0', []]]], ['dummy-npm-buzz@3.0.0', []], ['dummy-npm-foo@4.0.0', [['dummy-npm-buzz@2.0.0', []]]]]];
  var otherTree = ['npm-git-test@1.0.0', [['dummy-npm-bar@4.0.0', [['dummy-npm-buzz@3.0.0', []], ['dummy-npm-foo@3.0.0', []]]], ['dummy-npm-buzz@3.0.0', []], ['dummy-npm-foo@4.0.0', [['dummy-npm-buzz@2.0.0', []]]]]];
  ...
  test('correct versions are installed for git dependency', function(t) {
    ...
    npm.commands.install([], function(er) {
      ...
      npm.commands.ls([], true, function(er, result) {
        ...
        t.ok(deepEqual(simplified, oneTree) || deepEqual(simplified, otherTree), 'install tree is correct');
      });
    });
  });
})(require("process"));
