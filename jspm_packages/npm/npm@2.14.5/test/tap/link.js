...
(function(process) {
  ...
  var OPTS = {env: {'npm_config_userconfig': configPath}};
  var readJSON = {
    name: 'foo',
    version: '1.0.0',
    description: '',
  ...
  function setup() {
    ...
    mkdirp.sync(link);
    writeFileSync(path.join(link, 'package.json'), JSON.stringify(readJSON, null, 2));
    mkdirp.sync(linkInstall);
    writeFileSync(path.join(linkInstall, 'package.json'), JSON.stringify(installJSON, null, 2));
