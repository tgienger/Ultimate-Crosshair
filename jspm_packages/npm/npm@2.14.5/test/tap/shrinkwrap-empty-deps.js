...
(function(process) {
  ...
  test('returns a list of removed items', function(t) {
    mr({port: common.port}, function(er, s) {
      common.npm(['--registry', common.registry, '--loglevel', 'silent', 'shrinkwrap'], EXEC_OPTS, function(err, code, stdout, stderr) {
        ...
        fs.readFile(path.resolve(pkg, 'npm-shrinkwrap.json'), function(err, desired) {
          ...
          t.same({
            ...
            'version': '0.0.0',
            'dependencies': {}
          }, JSON.parse(desired), 'shrinkwrap handled empty deps without exploding');
          s.close();
          t.end();
        });
      });
