...
(function(process) {
  ...
  test("install from repo on 'OS X'", function(t) {
    common.npm(["install", "--prefix", target, "--global", pkg], EXEC_OPTS, function(err, code, stdout, stderr) {
      ...
      t.notOk(stderr.match(/Cannot read property '1' of null/), "no longer has cryptic error output");
      t.ok(stderr.match(/install-bad-man\.1\.lol is not a valid name/), "got expected error output");
      t.end();
    });
  });
  ...
  function setup() {
    ...
    fs.writeFileSync(resolve(pkg, "package.json"), JSON.stringify(json, null, 2) + "\n");
