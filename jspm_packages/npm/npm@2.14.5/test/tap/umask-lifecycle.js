...
(function(process) {
  ...
  var expected = ['', '> x@1.2.3 umask ' + path.join(__dirname, 'umask-lifecycle'), '> $npm_execpath config get umask && echo "$npm_config_umask" && node -pe "process.umask()"', '', sprintf('%04o', umask), sprintf('%04o', umask), sprintf('%d', umask), ''].join('\n');
