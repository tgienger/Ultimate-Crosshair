...
(function(process) {
  ...
  test('npm install', function(t) {
    common.npm(['install', '--cache', cache, '--registry', common.registry, '--loglevel', 'silent', '--production', 'false'], EXEC_OPTS, function(err, code, stdout, stderr) {
  ...
  test('npm install test-package', function(t) {
    common.npm(['install', 'test-package', '--cache', cache, '--registry', common.registry, '--loglevel', 'silent', '--production', 'false'], EXEC_OPTS, function(err, code, stdout, stderr) {
      t.ifErr(err, 'install finished successfully');
      t.notOk(code, 'exit ok');
      t.notOk(stderr, 'Should not get data on stderr: ' + stderr);
      t.end();
  ...
  test('npm prune', function(t) {
    common.npm(['prune', '--loglevel', 'silent', '--production', 'false'], EXEC_OPTS, function(err, code, stdout, stderr) {
  ...
  test('npm prune', function(t) {
    common.npm(['prune', '--loglevel', 'silent', '--production'], EXEC_OPTS, function(err, code, stderr) {
      t.ifErr(err, 'prune finished successfully');
      t.notOk(code, 'exit ok');
