...
(function(process) {
  ...
  test('No previous cache, init cache triggered by first search', function(t) {
    ...
    mr({
      port: common.port,
      plugin: mocks.allFutureUpdatedOnly
    }, function(err, s) {
      t.ifError(err, 'mock registry started');
      common.npm(['search', 'do not do extra search work on my behalf', '--registry', common.registry, '--cache', cache, '--loglevel', 'silent', '--color', 'always'], EXEC_OPTS, function(err, code) {
        s.close();
  ...
  test('previous cache, _updated set, should trigger since request', function(t) {
    ...
    mr({
      ...
      t.ifError(err, 'mock registry started');
      common.npm(['search', 'do not do extra search work on my behalf', '--registry', common.registry, '--cache', cache, '--loglevel', 'silly', '--color', 'always'], EXEC_OPTS, function(err, code) {
        s.close();
        t.equal(code, 0, 'search finished successfully');
        t.ifErr(err, 'search finished successfully');
        var cacheData = JSON.parse(fs.readFileSync(cacheJsonFile, 'utf8'));
  ...
  searches.forEach(function(search) {
    test(search.description + ' search in color', function(t) {
      ...
      mr({
        ...
        common.npm(['search', search.term, '--registry', common.registry, '--cache', cache, '--loglevel', 'silent', '--color', 'always'], EXEC_OPTS, function(err, code, stdout) {
          s.close();
          t.equal(code, 0, 'search finished successfully');
          t.ifErr(err, 'search finished successfully');
  ...
  function setupCache() {
    ...
    if (res)
      throw new Error('Creating cache file failed');
  }
  ...
  var allMock = {
    ...
    'generator-frontcow': {
      ...
      'dist-tags': {'latest': '0.1.19'},
      'maintainers': [{
        'name': 'bcabanes',
        'email': 'contact@benjamincabanes.com'
      }],
      'homepage': 'https://github.com/bcabanes/generator-frontcow',
      'keywords': ['sass', 'frontend', 'yeoman-generator', 'atomic', 'design', 'sass', 'foundation', 'foundation5', 'atomic design', 'bourbon', 'polyfill', 'font awesome'],
      'repository': {
        'type': 'git',
      ...
      'bugs': {'url': 'https://github.com/bcabanes/generator-frontcow/issues'},
      'license': 'MIT',
      'readmeFilename': 'README.md',
      'time': {'modified': '2014-10-03T02:26:18.406Z'},
      'versions': {'0.1.19': 'latest'}
    },
    'marko': {
      'name': 'marko',
      'description': 'Marko is an extensible, streaming, asynchronous, high performance, HTML-based templating language that can be used in Node.js or in the browser.',
      'dist-tags': {'latest': '1.2.16'},
      'maintainers': [{
        'name': 'pnidem',
      ...
      'keywords': ['templating', 'template', 'async', 'streaming'],
      'repository': {
        'type': 'git',
      ...
      'bugs': {'url': 'https://github.com/raptorjs/marko/issues'},
      'license': 'Apache License v2.0',
      'readmeFilename': 'README.md',
      'users': {'pnidem': true},
      'time': {'modified': '2014-10-03T02:27:31.775Z'},
      'versions': {'1.2.16': 'latest'}
    }
  };
})(require("process"));
