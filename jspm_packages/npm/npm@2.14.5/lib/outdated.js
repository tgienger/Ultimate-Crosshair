...
(function(process) {
  ...
  function outdated(args, silent, cb) {
    ...
    if (npm.config.get("depth") === Infinity)
      npm.config.set("depth", 0);
    outdated_(args, dir, {}, 0, function(er, list) {
      if (!list)
        list = [];
      if (er || silent || list.length === 0)
        return cb(er, list);
  ...
  function makePretty(p) {
    ...
    var columns = [dep, has || "MISSING", want, latest, dirToPrettyLocation(dir)];
    if (long)
      columns[5] = type;
    if (npm.color) {
      columns[0] = color[has === want ? "yellow" : "red"](columns[0]);
      columns[2] = color.green(columns[2]);
      columns[3] = color.magenta(columns[3]);
      columns[4] = color.brightBlack(columns[4]);
      if (long)
        columns[5] = color.brightBlack(columns[5]);
    }
    return columns;
  }
  ...
  function dirToPrettyLocation(dir) {
    return dir.replace(/^node_modules[/\\]/, "").replace(/[[/\\]node_modules[/\\]/g, " > ");
  }
  function makeParseable(list) {
    return list.map(function(p) {
      var dep = p[1],
          ...
          latest = p[4],
          type = p[6];
      var out = [dir, dep + "@" + want, (has ? (dep + "@" + has) : "MISSING"), dep + "@" + latest];
      if (long)
        out.push(type);
      return out.join(":");
    }).join(os.EOL);
  ...
  function outdated_(args, dir, parentHas, depth, cb) {
    ...
    readJson(path.resolve(dir, "package.json"), function(er, d) {
      ...
      var doUpdate = npm.config.get("dev") || (!npm.config.get("production") && !Object.keys(parentHas).length && !npm.config.get("global"));
      if (!er && d && doUpdate) {
    ...
    function next() {
      ...
      asyncMap(Object.keys(deps), function(dep, cb) {
        if (!long)
          return shouldUpdate(args, dir, dep, has, deps[dep], depth, cb);
  ...
  function shouldUpdate(args, dir, dep, has, req, depth, cb, type) {
    ...
    function skip(er) {
      if (er)
        return cb(er);
      outdated_(args, path.resolve(dir, "node_modules", dep), has, depth + 1, cb);
    }
