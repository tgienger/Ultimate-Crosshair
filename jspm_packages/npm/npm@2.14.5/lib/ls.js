...
(function(process) {
  ...
  ls.usage = "npm ls";
  ls.completion = require("./utils/completion/installed-deep");
  ...
  function filterByEnv(data) {
    ...
    Object.keys(data.dependencies).forEach(function(name) {
      ...
      if (production && keys.indexOf(name) !== -1)
        return;
      if (dev && keys.indexOf(name) === -1)
        return;
  ...
  function makeArchy_(data, long, dir, depth, parent, d) {
    ...
    if (data.invalid) {
      ...
      var invalid = "invalid";
      if (npm.color)
        invalid = color.bgBlack(color.red(invalid));
      out.label += " " + invalid;
    }
    if (data.peerInvalid) {
      var peerInvalid = "peer invalid";
      if (npm.color)
        peerInvalid = color.bgBlack(color.red(peerInvalid));
      out.label += " " + peerInvalid;
    }
    if (data.extraneous && data.path !== dir) {
      var extraneous = "extraneous";
      if (npm.color)
        extraneous = color.bgBlack(color.green(extraneous));
  ...
  function getExtras(data) {
    var extras = [];
    if (data.description)
      extras.push(data.description);
    if (data.repository)
      extras.push(data.repository.url);
    if (data.homepage)
      extras.push(data.homepage);
    if (data._from) {
      var from = data._from;
  ...
  function makeParseable_(data, long, dir, depth, parent, d) {
    if (data.hasOwnProperty("_found") && data._found !== true)
      return "";
    if (typeof data === "string") {
      if (data.depth < npm.config.get("depth")) {
        ...
        data = path.resolve(data.path || "", "node_modules", d || "") + (npm.config.get("long") ? ":" + d + "@" + JSON.stringify(data) + ":" + ":MAXDEPTH" : "");
      }
      return data;
    }
