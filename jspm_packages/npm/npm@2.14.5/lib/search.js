...
(function(process) {
  ...
  var npm = require("./npm"),
      ...
      updateIndex = require("./cache/update-index");
  ...
  function stripData(data) {
    return {
      ...
      time: data.time && data.time.modified && (new Date(data.time.modified).toISOString().split("T").join(" ").replace(/:[0-9]{2}\.[0-9]{3}Z$/, "")).slice(0, -5) || "prehistoric"
    };
  }
  ...
  function filterWords(data, args, notArgs) {
    ...
    for (var i = 0,
        l = args.length; i < l; i++) {
      if (!match(words, args[i]))
        return false;
    }
    for (i = 0, l = notArgs.length; i < l; i++) {
      if (match(words, notArgs[i]))
        return false;
    }
    return true;
  }
