...
var path = require("path"),
    ...
    addRemoteTarball = require("./add-remote-tarball"),
...
function getOnceFromRegistry(name, from, next, done) {
  ...
  mapToRegistry(name, npm.config, function(er, uri, auth) {
    ...
    if (!next)
      return log.verbose(from, key, "already in flight; waiting");
    else
      log.verbose(from, key, "not in flight; fetching");
...
function engineFilter(data) {
  ...
  if (!nodev || npm.config.get("force"))
    return data;
  Object.keys(data.versions || {}).forEach(function(v) {
    var eng = data.versions[v].engines;
    if (!eng)
      return;
    if (!strict && !data.versions[v].engineStrict)
      return;
...
function addNameVersion(name, v, data, cb) {
  ...
  if (!ver)
    return cb(new Error("Invalid version: " + v));
  var response;
  if (data) {
    response = null;
    return next();
  ...
  function next() {
    ...
    var dist = data.dist;
    if (!dist)
      return cb(new Error("No dist in " + data._id + " package"));
    if (!dist.tarball)
      return cb(new Error("No dist.tarball in " + data._id + " package"));
    if ((response && response.statusCode !== 304) || npm.config.get("force")) {
      return fetchit();
    }
    var pkgroot = cachedPackageRoot({
      name: name,
      version: ver
    });
