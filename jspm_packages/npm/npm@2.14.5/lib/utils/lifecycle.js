...
(function(process) {
  ...
  function lifecycle(pkg, stage, wd, unsafe, failOk, cb) {
    if (typeof cb !== "function")
      cb = failOk, failOk = false;
    if (typeof cb !== "function")
      cb = unsafe, unsafe = false;
    if (typeof cb !== "function")
      cb = wd, wd = null;
    while (pkg && pkg._data)
      pkg = pkg._data;
    if (!pkg)
      return cb(new Error("Invalid package data"));
    log.info(stage, pkg._id);
    if (!pkg.scripts || npm.config.get('ignore-scripts'))
      pkg.scripts = {};
  ...
  function makeEnv(data, prefix, env) {
    ...
    keys.forEach(function(i) {
      ...
      if (value instanceof Stream || Array.isArray(value))
        return;
      if (i.match(/umask/))
        value = umask.toString(value);
      if (!value)
        value = "";
