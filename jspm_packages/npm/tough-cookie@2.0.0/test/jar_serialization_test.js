    ...
    switch (type) {
      ...
      case 'intOrInf':
        if (cookie[prop] === 'Infinity' || cookie[prop] === '-Infinity') {
          ...
          assert(isInteger(cookie[prop]), "serialized property isn't integer: " + prop);
        }
        break;
      case 'isoDate':
...
vows.describe('CookieJar serialization').addBatch({"Assumptions:": {"serializableProperties all accounted for": function() {
      var actualKeys = Cookie.serializableProperties.concat([]);
      actualKeys.sort();
      var expectedKeys = Object.keys(serializedCookiePropTypes);
      expectedKeys.sort();
      assert.deepEqual(actualKeys, expectedKeys);
    }}}).addBatch({"For Stores without getAllCookies": {
    topic: function() {
      var store = new Store();
  ...
  }}).addBatch({"With a moderately-sized store": {
    topic: function() {
      setUp(this);
