...
exports.assertType = function(node, type, param) {
  ...
  var actual = node.nodeName,
      msg = 'expected "' + param + '" to be a ' + type + ', but got ' + actual + ':' + node;
  ...
  function compile(arr, i) {
    if (i) {
      arr[i].forEach(function(selector) {
        if (!leaveHidden && selector.val.match(HIDDEN_SELECTOR_RX))
          return;
      ...
      arr[0].forEach(function(selector) {
        ...
        if (~str.indexOf('&'))
          str = str.replace(PARENT_SELECTOR_RX, '$1').replace(ESCAPED_PARENT_SELECTOR_RX, '&').trim();
        if (!str.length)
          return;
