...
(function(process) {
  ...
  var parseName = /^(?:@([^/]+?)[/])?([^/]+?)$/;
  var nameAt = /^(@([^/]+?)[/])?([^/]+?)@/;
  var debug = util.debuglog ? util.debuglog("npa") : /\bnpa\b/i.test(process.env.NODE_DEBUG || "") ? function() {
  ...
  function npa(arg) {
    ...
    if (nameparse && validName(nameparse[3]) && (!nameparse[2] || validName(nameparse[2]))) {
    ...
    if (isWindows && urlparse && urlparse.protocol && urlparse.protocol.match(/^[a-zA-Z]:$/)) {
    ...
    if (res.name) {
      ...
      if (p && validName(p[2]) && (!p[1] || validName(p[1]))) {
        res.type = "range";
        res.spec = "*";
        res.rawSpec = "";
        res.name = arg;
  ...
  function parseUrl(res, arg, urlparse) {
    ...
    if (gitHost) {
      res.type = "hosted";
      res.spec = gitHost.toString(), res.hosted = {
        type: gitHost.type,
        ssh: gitHost.ssh(),
