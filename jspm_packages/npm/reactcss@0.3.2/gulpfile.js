...
(function(process) {
  ...
  config = {docs: {
      ...
      resolve: {
        alias: {'reactcss': path.resolve(__dirname, './lib/react-css.js')},
        extensions: ['', '.js', '.coffee', '.jsx', '.cjsx'],
        fallback: [path.resolve(__dirname, './modules')]
      },
      plugins: [new webpack.HotModuleReplacementPlugin({quiet: true}), new webpack.NoErrorsPlugin()],
      devtool: 'eval',
      quiet: true
    }};
  ...
  gulp.task('bundle', function(done) {
    gulp.src('./src/**/*.coffee').pipe(coffee({bare: true})).pipe(uglify()).pipe(gulp.dest('lib'));
    done();
  });
  gulp.task('static', function(done) {
    prodConfig = {
      entry: {
        home: './docs/index.coffee',
        documentation: './docs/documentation/index.coffee'
      },
      output: {
        path: path.join(__dirname, 'docs/build'),
        filename: '[name].js',
      ...
      plugins: [new webpack.DefinePlugin({'process.env': {NODE_ENV: JSON.stringify('production')}}), new webpack.optimize.DedupePlugin(), new webpack.optimize.CommonsChunkPlugin("common.js")],
      devtool: 'eval',
      quiet: true
    };
